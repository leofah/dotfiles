...
# Copyright (c) 2022 - 2023 Leo Fahrbach
#
# Released under GPLv3 or later.
#
#_this config is supposed to be _unpersonalized_!
...
# locales
# test availability of these with `locale -a`
...
# prepend the current date before each line
logdate () {
...
compdef tryping=ping 2>/dev/null
...
PROMPT="%B%{$fg[green]%}%n%{$fg[cyan]%}@%{$fg[red]%}%m%b %{$fg[blue]%}%~ %{$fg[green]%}%1v%2v%{$reset_color%}%# "
RPROMPT="%3v%4v%{$reset_color%}[%{$fg[cyan]%}%?%{$reset_color%}]%1v%{$fg[green]%}:%{$fg[yellow]%}%l%{$reset_color%}"
